    ...
    useEffect(() => {
        ...
        async function getScheduleForPopulate() {
            try {
                ...
                const formattedTime = `${utcTime.getFullYear()}-${padZero(utcTime.getMonth() + 1)}-${padZero(utcTime.getDate())}T${padZero(utcTime.getHours())}:${padZero(utcTime.getMinutes())}`;
                setScheduleInput({
                ...
                {loadingPopulate ? "Loading..." : (
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="item">
                            ...
                            <input style={{ backgroundColor: "white", color: 'black' }} name="time" type="datetime-local" value={scheduleInput.time} id="time" onChange={handleChange} />
                        </div>
